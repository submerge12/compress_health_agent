...
export const MIN_SCHEMA_VERSION = 26;
...
export async function assertSchemaReady(databaseUrl: string): Promise<void> {
  ...
  try {
    ...
    const requiredObjects: Array<[string, "table" | "column" | "index"]> = [
      ...
      ["daily_health_state_projection.schema_version", "column"],
      ["sensitive_payloads", "table"],
      ["sensitive_payload_access_grants", "table"],
      ["agent_runs.objective_payload_id", "column"],
      ["agent_runs.response_summary_payload_id", "column"],
      ["plan_versions", "table"],
      ...
      ["substitution_proposals", "table"],
      ["training_cycle_instances", "table"],
      ["training_cycle_positions", "table"],
      ["agent_runs", "table"],
      ["agent_run_steps", "table"],
      ["mcp_pending_input_requests", "table"],
      ["mcp_write_receipts", "table"],
      ...
      ["bmr_profiles.goal_weight_kg", "column"],
      ["bmr_profiles.training_cadence", "column"],
      ["bmr_profiles.training_split", "column"],
      ["bmr_profiles_user_effective_date_idx", "index"],
    ];
